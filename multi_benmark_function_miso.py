...
def _to_dense_vector(x):
    """Convert 1D sparse/dense array-like to dense numpy vector."""
...
def _build_spatial_connectivity(coords, n_neighbors=6, include_self=False):
    """
    Build a binary kNN spatial connectivity matrix W of shape (n_cells, n_cells).
    Returns scipy CSR sparse matrix.
    """
    ...
    # ask for +1 because the first neighbor is usually self
    ...
    W = sparse.coo_matrix((data, (rows, cols)), shape=(n_cells, n_cells)).tocsr()

    # symmetrize to mimic an undirected spatial graph
    W = W.maximum(W.T)
...
def _moran_i(x, W):
    """
    Moran's I for a single gene expression vector x and sparse weight matrix W.
    """
    x = _to_dense_vector(x)
...
def _geary_c(x, W):
    """
    Geary's C for a single gene expression vector x and sparse weight matrix W.
    """
    x = _to_dense_vector(x)
    ...
    # Geary numerator: sum_ij w_ij (x_i - x_j)^2
    W_coo = W.tocoo()
    diff_sq = (x[W_coo.row] - x[W_coo.col]) ** 2
    num = np.sum(W_coo.data * diff_sq)

    C = ((n - 1) / (2 * W_sum)) * (num / denom)
    return float(C)


def marker_score_no_squidpy(
    adata_orig,
    domain_key,
    top_n=5,
    ...
    """
    Squidpy-free version of marker_score.

    Workflow:
    1) filter groups with enough samples
    2) normalize + log1p
    3) rank marker genes per domain with scanpy
    4) collect top marker genes
    5) build kNN spatial graph from adata.obsm[spatial_key]
    6) compute Moran's I and Geary's C for each marker gene
    7) return median Moran's I and median Geary's C across marker genes
    """
    ...
    if len(valid_groups) < 2:
        logging.warning(
            f"marker_score: Not enough groups (found {len(valid_groups)}) with >= {min_samples_per_group} samples "
            f"for domain '{domain_key}'. Skipping."
        )
        return np.nan, np.nan
    ...
    if adata_filtered.shape[0] < 2 or len(adata_filtered.obs[domain_key].cat.categories) < 2:
        logging.warning(
            f"marker_score: adata_filtered has < 2 samples or < 2 categories for domain '{domain_key}'. Skipping."
        )
        return np.nan, np.nan

    # -----------------------------
    # 2. Make var names safe
    # -----------------------------
    if not adata_filtered.var_names.empty:
        if (
            not pd.api.types.is_string_dtype(adata_filtered.var_names.dtype)
            and not all(isinstance(x, str) for x in adata_filtered.var_names)
        ):
            logging.info(f"marker_score: Converting var_names to string for domain '{domain_key}'.")
            try:
                adata_filtered.var_names = adata_filtered.var_names.astype(str)
            except Exception as e:
                logging.error(
                    f"marker_score: Could not convert var_names to string for domain '{domain_key}': {e}. Skipping."
                )
                return np.nan, np.nan

    if not adata_filtered.var_names.is_unique:
        logging.info(f"marker_score: Making var_names unique for domain '{domain_key}'.")
    ...
    is_all_zero = X_data.nnz == 0 if sparse.issparse(X_data) else np.all(X_data == 0)

    is_constant = False
    if adata_filtered.shape[0] > 0 and adata_filtered.shape[1] > 0:
        if sparse.issparse(X_data):
            X_dense = X_data.toarray()
            is_constant = np.all(X_dense == X_dense.flat[0])
        else:
            is_constant = np.all(X_data == X_data.flat[0])
    ...
    if n_genes_param == 0:
        logging.warning(f"marker_score: adata_filtered has 0 genes for domain '{domain_key}'.")
        return np.nan, np.nan
    ...
    # -----------------------------
    # 6. Collect top marker genes
    # -----------------------------
    selected_genes = []
    if "names" in adata_filtered.uns["rank_genes_groups"]:
        all_ranked_genes = []
        names_struct = adata_filtered.uns["rank_genes_groups"]["names"]

        for group_name in names_struct.dtype.names:
            group_genes_array = names_struct[group_name]
            try:
                gene_list = list(group_genes_array[:top_n])
                all_ranked_genes.extend(
                    [str(g) for g in gene_list if g is not None and str(g) != "nan"]
                )
            except Exception as e:
                logging.warning(
                    f"marker_score: Error processing genes for group '{group_name}', domain '{domain_key}': {e}"
                )

        selected_genes = list(np.unique(all_ranked_genes))

    if not selected_genes:
        logging.warning(f"marker_score: No marker genes found for domain '{domain_key}'.")
        return np.nan, np.nan

    valid_selected_genes = [g for g in selected_genes if g in adata_filtered.var_names]
    if not valid_selected_genes:
        logging.warning(
            f"marker_score: No valid marker genes found in adata_filtered.var_names for domain '{domain_key}'."
        )
        return np.nan, np.nan
    ...
    # -----------------------------
    if spatial_key not in adata_filtered.obsm:
        logging.error(
            f"marker_score: '{spatial_key}' coordinates not found in adata.obsm for domain '{domain_key}'."
        )
        return np.nan, np.nan

    coords = adata_filtered.obsm[spatial_key]
    if coords.shape[0] != adata_filtered.shape[0]:
        logging.error(
            f"marker_score: Number of spatial coordinates does not match n_obs for domain '{domain_key}'."
        )
        return np.nan, np.nan

    W = _build_spatial_connectivity(coords, n_neighbors=n_neighbors, include_self=False)
    if W.nnz == 0:
        logging.warning(f"marker_score: Spatial graph is empty for domain '{domain_key}'.")
        return np.nan, np.nan

    # -----------------------------
    # 8. Compute Moran's I / Geary's C for each marker gene
    # -----------------------------
    moran_scores = []
    geary_scores = []

    for gene in valid_selected_genes:
        x = adata_filtered[:, gene].X
        x = _to_dense_vector(x)

        # skip constant genes
        if np.all(np.isnan(x)) or np.nanvar(x) == 0:
            continue

        try:
            I = _moran_i(x, W)
            if not np.isnan(I):
                moran_scores.append(I)
        except Exception as e:
            logging.warning(f"marker_score: Moran failed for gene {gene} in domain '{domain_key}': {e}")

        try:
            C = _geary_c(x, W)
            if not np.isnan(C):
                geary_scores.append(C)
        except Exception as e:
            logging.warning(f"marker_score: Geary failed for gene {gene} in domain '{domain_key}': {e}")

    moranI = np.nanmedian(moran_scores) if len(moran_scores) > 0 else np.nan
    gearyC = np.nanmedian(geary_scores) if len(geary_scores) > 0 else np.nan
