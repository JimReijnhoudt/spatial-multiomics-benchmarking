...
def run_vertical_spatialmeta(iteration_dir, n_latent, hidden_stacks):
    ...
    # ## Benchmarking metrics
    # 
    # ### Adding ground truth data for biological conservation score
    ...
    ari = compute_ARI(joint_adata, gt_key=GT_KEY, pred_key=PRED_KEY)
    nmi = compute_NMI(joint_adata, gt_key=GT_KEY, pred_key=PRED_KEY)
    print(f"ARI: {ari:.4f}")
    print(f"NMI: {nmi:.4f}")
...
# Run SpatialMETA
peak_memory, results = memory_usage((run_vertical_spatialmeta, (), {"iteration_dir": iteration_dir, "n_latent": n_latent, "hidden_stacks": hidden_stacks}), retval=True, max_usage=True)
